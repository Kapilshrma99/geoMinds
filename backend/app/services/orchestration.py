...
class PlannerAgent(WorkflowAgent):
    ...
    def run(self, context: AgentContext) -> None:
        context.emit(self.name, "Planning sequential workflow with shared state for document, GIS, conflict, risk, and report stages.", status="running")
        ...
        context.emit(self.name, "Delegation plan created using ADK-style sequential orchestration and shared state.", metadata={"sub_agents": context.state["workflow_plan"]})
...
class DocumentAgent(WorkflowAgent):
    ...
    def run(self, context: AgentContext) -> None:
        ...
        context.emit(self.name, "Chunking extracted text and generating embeddings for the pgvector knowledge store.", status="running")
...
class GISAgent(WorkflowAgent):
    ...
    def run(self, context: AgentContext) -> None:
        context.emit(self.name, "Matching extracted property state against parcel records and spatial metadata.", status="running")
        ...
        if match:
            context.emit(self.name, f"Matched property to parcel {match.parcel_id} with confidence score {match.match_score:.0f}.")
        else:
            context.emit(self.name, "No reliable parcel match found from the available cadastral signals.", status="completed")
...
class ConflictAgent(WorkflowAgent):
    ...
    def run(self, context: AgentContext) -> None:
        context.emit(self.name, "Checking overlap, ownership mismatch, duplicate khasra, missing geometry, and area anomalies.", status="running")
        extracted: ExtractedPropertyData = context.state["extracted"]
        ...
        context.emit(self.name, f"Conflict scan finished with {len(conflicts)} findings, including {high_or_medium} elevated-risk signals.")
...
class RiskAgent(WorkflowAgent):
    ...
    def run(self, context: AgentContext) -> None:
        context.emit(self.name, "Computing baseline risk score and preparing evidence for Gemini reasoning.", status="running")
        extracted: ExtractedPropertyData = context.state["extracted"]
        match: PropertyMatch | None = context.state.get("match")
...
class ReportAgent(WorkflowAgent):
    ...
    def run(self, context: AgentContext) -> None:
        context.emit(self.name, "Generating the final intelligence report from shared workflow state.", status="running")
        extracted: ExtractedPropertyData = context.state["extracted"]
        match: PropertyMatch | None = context.state.get("match")
        conflicts: list[ConflictReport] = context.state.get("conflicts", [])
        ...
        context.emit(self.name, f"Property intelligence report #{report.id} generated with {report.risk_level} risk rating ({report.risk_score}/100).")
